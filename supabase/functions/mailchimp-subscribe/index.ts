...
const BodySchema = z.object({
  ...
  tag: z.enum(['newsletter-only', 'lovable-member-signup']),
  first_name: z.string().max(100).optional(),
  last_name: z.string().max(100).optional(),
});

function md5(input: string): string {
  // Minimal MD5 implementation (Mailchimp requires md5(lowercase(email)) as subscriber hash)
  ...
  function sb(s: string) {
    const nb = ((s.length + 8) >> 6) + 1, bl: number[] = new Array(nb * 16).fill(0);
    let i = 0;
    for (; i < s.length; i++) bl[i >> 2] |= s.charCodeAt(i) << ((i % 4) * 8);
    bl[i >> 2] |= 0x80 << ((i % 4) * 8);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey || !audienceId) {
      return new Response(JSON.stringify({ error: 'Mailchimp er ikke konfigurert' }), {
        status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    const dc = apiKey.split('-')[1];
    if (!dc) {
      return new Response(JSON.stringify({ error: 'Ugyldig Mailchimp API-nøkkel' }), {
        status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!parsed.success) {
      return new Response(JSON.stringify({ error: 'Ugyldig forespørsel', details: parsed.error.flatten().fieldErrors }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    const { email, tag, first_name, last_name } = parsed.data;
    ...
    // Upsert member
    const upsertRes = await fetch(`${base}/members/${hash}`, {
      method: 'PUT',
    ...
    if (!upsertRes.ok) {
      ...
      console.error('Mailchimp upsert failed:', upsertRes.status, text);
      return new Response(JSON.stringify({ error: 'Kunne ikke registrere i Mailchimp' }), {
